          ...
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="(goods,index) in  goodsList" :key="goods.id">
                <div class="list-wrap">
                  <div class="p-img">
                    <router-link :to="`/detail/${goods.id}`" target="_self"><img :src="goods.defaultImg" /></router-link>
                  </div>
                  <div class="price">
                    <strong>
                      <em>¥ </em>
...
<!--          分页器-->
          <Pagination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize"  :continues="5" @getPageNo="getPageNo"/>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  beforeMount() {
      Object.assign(this.searchParams,this.$route.query,this.$route.params)  //将params和query两者和searchParams整合
  },
    name: 'Search',
    components: {
      SearchSelector
    ...
    methods:{
      ...
      removeCategoryName(){
        console.log("删除之前的",this.$route)

        this.searchParams.categoryName=undefined;
        this.searchParams.category1Id=undefined;
        this.searchParams.category2Id=undefined;
        this.searchParams.category3Id=undefined;
        this.getData()//通过dispatce触发actions mutations 等一系列操作把数据从服务器返回到store仓库中
        //路由跳转 自己眺自己
        console.log("删除之后的",this.$route)
    ...
    watch:{
      $route(){
        ...
        //再次发起AJAX请求
        this.getData();
        console.log("我是watch的",this.searchParams);
